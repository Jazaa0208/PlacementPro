...
const Aptitude = () => {
  ...
  const handlePracticeSubmit = (selected) => {
    ...
    if (selected === q.correct_answer) {
      if (currentQuestionIndex === EXPECTED_PRACTICE_QUESTION_COUNT - 1) {
        completePractice();
      } else {
  ...
  const handleTestSubmit = async () => {
    if (Object.keys(testAnswers).length !== EXPECTED_TEST_QUESTION_COUNT) {
  ...
  const getOptionClasses = (optionChar) => {
    const isSelected =
      practiceAnswers[currentQuestionIndex] === optionChar;
    const isCorrect =
      practiceQuestions[currentQuestionIndex]?.correct_answer === optionChar;
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-purple-900 relative">
      ...
      <div className="ml-72 flex-1 p-8">
        ...
        {/* Error */}
        {error && (
          <div className="bg-red-500/20 text-red-200 p-4 rounded-xl mb-6 border border-red-500/50 flex items-center gap-2">
            <FaSync className="animate-spin" /> {error}
          </div>
        )}
          ...
          <div className="bg-white/10 rounded-3xl p-8 border border-indigo-500/30">
            ...
            <div className="mb-6">
              <div className="text-indigo-200 text-sm mb-2">
                Question {currentQuestionIndex + 1} of {EXPECTED_PRACTICE_QUESTION_COUNT}
              </div>
              <div className="w-full bg-gray-700 h-3 rounded-full">
                <div
                  className="bg-green-500 h-3 rounded-full transition-all"
                  style={{
                    width: `${((currentQuestionIndex + 1) / EXPECTED_PRACTICE_QUESTION_COUNT) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            {showSolution && (
              <div className="mt-6 bg-red-800/30 border border-red-500/50 p-5 rounded-xl">
                ...
                >
                  {currentQuestionIndex < EXPECTED_PRACTICE_QUESTION_COUNT - 1
                    ? "Next Question"
                    : "Finish Practice"}
                </button>
          ...
          <div className="bg-white/10 rounded-3xl p-8 border border-indigo-500/30">
            ...
            <div className="text-center mt-8">
              <button
                ...
                disabled={
                  Object.keys(testAnswers).length !== EXPECTED_TEST_QUESTION_COUNT
                }
              ...
              >
                ...
                Submit Test ({Object.keys(testAnswers).length}/{EXPECTED_TEST_QUESTION_COUNT})
              </button>
            </div>
          ...
          <div className="bg-white/10 rounded-3xl p-8 border border-indigo-500/30 text-center">
            ...
            <p className="text-indigo-200 text-2xl mb-6">
              ...
              <span className="text-5xl font-black text-white">
                {testScore}/{EXPECTED_TEST_QUESTION_COUNT}
              </span>
            </p>

            <p
              className={`text-3xl font-bold mb-8 ${
                testScore >= MIN_PASS_SCORE ? "text-green-400" : "text-red-400"
              }`}
            >
              {testScore >= MIN_PASS_SCORE
